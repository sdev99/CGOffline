...
export class HttpConfigInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    if (!this.sharedDataService.dedicatedMode) {
      if (
        this.accountService.userValue?.mobileAppLanguageID ||
      ...
      catchError((err: HttpErrorResponse) => {
        ...
        if (error) {
          ...
          if (
            ...
            error.ResponseException.ValidationErrors &&
            error.ResponseException.ValidationErrors.length > 0
          ) {
            error.ResponseException.ValidationErrors.map((data) => {
              const errorField =
                this.sharedDataService.companyLangaugeTranslations[
                  data.Field
                ] || data.Field;
              const errorData =
                this.sharedDataService.companyLangaugeTranslations[
              ...
              errorMessage =
                errorMessage + errorField + " : " + errorData + "\n";
            });
          }
