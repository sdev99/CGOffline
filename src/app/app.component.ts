...
export class AppComponent {
    ...
    initializeApp() {
        this.platform.ready().then(async () => {
            ...
            this.sharedDataService.indexDbStorage = await this.storage.create();
            ...
            App.addListener("appUrlOpen", (data: any) => {
                this.ngZone.run(() => {
                    ...
                    if (url.indexOf("ResetPassword") !== -1) {
                        ...
                        const code = UtilService.getQueryStringValue(
                            url,
                            "code"
                        );
                        this.router.navigate(["forgot-password-reset"], {
    ...
    checkForAccessKey = async (callBack) => {
        if (
            !localStorage.getItem(EnumService.LocalStorageKeys.API_ACCESS_KEY)
        ) {
    ...
    configureAppForDedicatedMode = async (data: Response = null) => {
        ...
        if (data) {
            ...
            if (
                this.sharedDataService.dedicatedModeAssignedEntities &&
                this.sharedDataService.dedicatedModeAssignedEntities.length ===
                    1
            ) {
    ...
    configureAppForPersonalMode = async () => {
        ...
        if (!this.sharedDataService.isNavigationTypeDeepLink) {
            ...
            if (user?.userId && !user.isMobileSessionExpiration) {
                ...
                if (
                    !this.sharedDataService.isLoginAfterAppOpen &&
                    user?.companyID
                ) {
                    this.accountService
                        ...
                        .subscribe((response: Response) => {
                            if (
                                ...
                                if (
                                    response.Result?.isMobileSessionExpiration
                                ) {
