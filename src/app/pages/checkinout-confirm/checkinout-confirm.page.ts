...
export class CheckinoutConfirmPage implements OnInit {
  ...
  ngOnInit() {
    if (this.locationCheckType === EnumService.ConfirmForCheckType.CheckIn) {
      ...
      this.locationCheckType === EnumService.ConfirmForCheckType.CheckOut
    ) {
      this.checkOutForCheckedInDetail =
        this.sharedDataService.checkOutForCheckedInDetail;
    }
  }
  ...
  async onContinue() {
    const nowCheckoutOutKey =
      "PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.YOU_HAVE_NOW_CHECKED_OUT";
    const continueKey = "SHARED_TEXT.CONTINUE";
    const cannotCheckoutKey =
      "PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.YOU_CANNOT_CHECK_OUT";
    this.translateService
      .get([nowCheckoutOutKey, continueKey, cannotCheckoutKey])
      .subscribe((res) => {
        if (
          this.locationCheckType === EnumService.ConfirmForCheckType.CheckOut
        ) {
          ...
          this.apiService
            .insertCheckOutDetails({
              userCheckInDetailID:
                this.checkOutForCheckedInDetail.userCheckInDetailID,
              ...
              (response: Response) => {
                ...
                if (
                  ...
                  this.navCtrl.navigateForward(["/checkin-success"], {
                    queryParams: {
                      message: res[nowCheckoutOutKey],
                      nextPage: "/tabs/dashboard",
                      actionBtnTitle: res[continueKey],
                    },
                  });
                } else {
                  this.navCtrl.navigateForward(["/checkin-fail"], {
                    queryParams: {
                      title: res[cannotCheckoutKey],
          ...
          if (
            ...
            this.checkinDetail.checkInInductionItems &&
