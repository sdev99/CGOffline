...
export class DashboardQrscanPage implements OnInit {
    ...
    const onSuccess = (res) => {
      ...
      if (
        ...
        (!this.isOnlyInventryItemHasHav ||
          (result.entityType === EnumService.SelectedQRCodeType.InventoryItem &&
            result.isHAVSData))
      ) {
        ...
        .then((res) => {
          if (res) {
            ...
            this.translateService
              ...
              .subscribe((transRes) => {
                this.utilService.showAlert(
                  transRes["SHARED_TEXT.ERRORS.QR_CODE_NOT_VALID"],
    ...
    if (this.sharedDataService.offlineMode) {
      this.offlineManagerService.getUserDetailByQRCode(qrCode).then((res) => {
        ...
        if (
          ...
          this.translateService
            .get("SHARED_TEXT.ERRORS.USER_NOT_VALID_FOR_ANY_OPERATION")
            .subscribe((res) => {
              this.utilService.showAlert(res, "", () => {
                this.scan();
              });
        ...
        this.sharedDataService.getCheckinDetailsForDedicatedMode(
          ...
          ({ ischeckInPersonalQRNotAllowed }) => {
            if (ischeckInPersonalQRNotAllowed) {
              this.translateService
                .get("SHARED_TEXT.ERRORS.CANNOT_CHECKIN_WITH_PERSONAL_QR_CODE")
                .subscribe((res) => {
                  this.utilService.showAlert(res, "", () => {
                    this.scan();
                  });
  ...
  async showCheckedInLocations(getEntityIds) {
    this.translateService
      .get(["SHARED_TEXT.CANCEL", "PAGESPECIFIC_TEXT.QR_SCANER.CHOOSE_PLACE"])
      .subscribe(async (res) => {
