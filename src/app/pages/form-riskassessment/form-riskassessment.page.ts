...
export class FormRiskassessmentPage implements OnInit {
  ...
  async openDrodownForAddUserGroup(
    ...
    const valuesList = hazard[valuesHoldKey]
      ? this.convertStringToArray(hazard[valuesHoldKey], isValueTypeNumber)
      : [];
            ...
            if (status) {
              try {
                ...
                task.hazardAnswers.map(
                  (hazardAnswer: RAhazardAswerObject, key) => {
            ...
            if (status) {
              try {
                ...
                hazard.controlMeasureAnswers.map(
                  (controlMeasureAnswer: RAcontrolMeasureAnswerObject, key) => {
