...
export class CheckinWorkpermitPage implements OnInit {
	...
	getDedicatedModeAvailableWorkPermits(itemDetail: FormItem, callBack = null) {
		...
		if (this.sharedDataService.offlineMode) {
			...
			this.offlineManagerService.getDeviceAvailableWorkPermits(dedicatedModeLocationUse, itemDetail.formFolderID).then((res: any) => {
	...
	async openForm(form: FormItem) {
		...
		if (this.sharedDataService.dedicatedMode) {
			...
			if (this.sharedDataService.offlineMode) {
				...
				this.apiService.getDedicatedModeSignOffFormDetail(form.formID).subscribe(
					(response: Response) => {
						this.utilService.hideLoading();
						if (response.StatusCode === EnumService.ApiResponseCode.RequestSuccessful) {
