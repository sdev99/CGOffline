...
export class DocumentOpenchoiceDmPage implements OnInit {
	...
	async getDocumentSignOffDetail() {
		if (this.sharedDataService.offlineMode) {
			const signOffDocumentDetail: any = this.documentDetail as DeviceAvailableDocumentDetail;
			...
			this.apiService.getActivitySignOffDocumentDetail(this.documentDetail?.documentID).subscribe(
				(response: Response) => {
