...
export class MyProfileDetailsPage implements OnInit {
  ...
  constructor(
    ...
    if (
      localStorage.getItem(EnumService.LocalStorageKeys.LOGIN_WITH_OKTA) ===
      "true"
    ) {
      this.isLoggedInWithOkta = true;
    }
    if (this.profile) {
      this.offsetData = UtilService.findObj(
        sharedDataService.timeZoneList,
        "timeZoneID",
        this.profile.timeZoneID
      );
      this.languageData = UtilService.findObj(
        sharedDataService.companyLanguageList,
        "companyLanguageID",
