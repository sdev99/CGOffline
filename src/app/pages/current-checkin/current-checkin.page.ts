...
export class CurrentCheckinPage implements OnInit {
	...
	getPersonalModeAvailableForms = async (callBack = null) => {
		this.apiService.getPersonalModeAvailableForms(this.user?.userId, this.user.companyID).subscribe(
			(response: Response) => {
	...
	getPersonalModeAvailableDocuments = async (callBack = null) => {
		this.apiService.getPersonalModeAvailableDocuments(this.user?.userId, this.user.companyID).subscribe(
			(response: Response) => {
				if (response.StatusCode === EnumService.ApiResponseCode.RequestSuccessful) {
	...
	getPersonalModeAvailableWorkPermits = async (callBack = null) => {
		this.apiService.getPersonalModeAvailableWorkPermits(this.user?.userId, this.user.companyID).subscribe(
			(response: Response) => {
				if (response.StatusCode === EnumService.ApiResponseCode.RequestSuccessful) {
	...
	async openForm(form: FormItem) {
		...
		this.apiService.getSignOffFormDetail(this.user?.userId, form?.formID, place?.locationID, place?.projectID, place?.inventoryItemID).subscribe(
			(response: Response) => {
				this.utilService.hideLoading();
				if (response.StatusCode === EnumService.ApiResponseCode.RequestSuccessful) {
	...
	openFormFolder(item) {
		this.navCtrl.navigateForward(['/tabs/current-checkin/folder-content-list'], {
