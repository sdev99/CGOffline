...
export class ActivityDetailPage implements OnInit {
	...
	getActivityDetail = (callBack = null) => {
		...
		if (this.activityListItem) {
			this.apiService.getActivityDetail(this.activityListItem.activityIndividualID).subscribe(
				(response: Response) => {
	...
	async openForm() {
		if (this.activityListItem) {
			...
			this.apiService.getActivitySignOffFormDetail(this.user?.userId, this.activityListItem?.formID, this.activityListItem?.activityIndividualID).subscribe(
				(response: Response) => {
					this.utilService.hideLoading();
					if (response.StatusCode === EnumService.ApiResponseCode.RequestSuccessful) {
	...
	async openDocument() {
		if (this.activityListItem) {
			...
			this.apiService.getActivitySignOffDocumentDetail(this.activityListItem?.documentID).subscribe(
				(response: Response) => {
					this.utilService.hideLoading();
					if (response.StatusCode === EnumService.ApiResponseCode.RequestSuccessful) {
