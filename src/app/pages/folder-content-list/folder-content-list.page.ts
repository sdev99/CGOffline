...
export class FolderContentListPage implements OnInit {
	...
	getPersonalModeAvailableForms(formItem: FormItem, callBack = null) {
		this.apiService.getPersonalModeAvailableForms(this.user?.userId, this.user.companyID, formItem.formFolderID).subscribe(
			(response: Response) => {
	...
	getPersonalModeAvailableDocuments(documentDetail: DocumentDetail, callBack = null) {
		this.apiService.getPersonalModeAvailableDocuments(this.user?.userId, this.user.companyID, documentDetail.folderID).subscribe(
			(response: Response) => {
				if (response.StatusCode === EnumService.ApiResponseCode.RequestSuccessful) {
