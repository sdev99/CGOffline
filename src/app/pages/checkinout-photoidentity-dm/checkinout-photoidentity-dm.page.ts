...
export class CheckinoutPhotoidentityDmPage implements OnInit {
	...
	constructor(
		...
		if (
			sharedDataService.dedicatedModeCapturePhotoFor === EnumService.DedicatedModeCapturePhotoForType.Auth ||
			sharedDataService.dedicatedModeCapturePhotoFor === EnumService.DedicatedModeCapturePhotoForType.LocationPhoto
		) {
		...
		if (this.sharedDataService.offlineMode) {
			if (
				(this.sharedDataService.dedicatedModeCapturePhotoFor === EnumService.DedicatedModeCapturePhotoForType.Auth ||
					this.sharedDataService.dedicatedModeCapturePhotoFor === EnumService.DedicatedModeCapturePhotoForType.LocationPhoto) &&
				this.sharedDataService.dedicatedModeProcessType === EnumService.DedicatedModeProcessTypes.CheckinOut
			) {
				this.sharedDataService.dedicatedModeCapturedSelfieForCheckinProcess = { fileName: fileName, binaryFile: this.photoCaptured };
				this.processToNextScreen({ fileName: fileName, binaryFile: this.photoCaptured });
			} else {
				this.processToNextScreen({ fileName: fileName, binaryFile: this.photoCaptured });
			}
		} else {
	...
	processToNextScreen = ({ photoName, fileName, binaryFile }: { photoName?: string; fileName?: string; binaryFile?: string } = {}) => {
		if (this.sharedDataService.dedicatedModeCapturePhotoFor === EnumService.DedicatedModeCapturePhotoForType.Auth) {
				...
				case EnumService.DedicatedModeProcessTypes.CheckinOut: {
					switch (this.sharedDataService.checkinoutDmAs) {
						case EnumService.CheckInType.AS_GUEST:
							if (this.sharedDataService.offlineMode) {
								this.sharedDataService.dedicatedModeGuestDetail.guestPhotoFileName = fileName;
								this.sharedDataService.dedicatedModeGuestDetail.guestPhotoBinaryFile = binaryFile;
							} else {
			...
			switch (this.sharedDataService.checkinoutDmAs) {
				case EnumService.CheckInType.AS_GUEST:
					if (this.sharedDataService.offlineMode) {
						this.sharedDataService.checkInPostData.guestPhotoFileName = fileName;
						this.sharedDataService.checkInPostData.guestPhotoBinaryFile = binaryFile;
					} else {
				...
				case EnumService.CheckInType.MY_NAME:
					if (this.sharedDataService.offlineMode) {
						this.sharedDataService.checkInPostData.userPhotoFileName = fileName;
						this.sharedDataService.checkInPostData.userPhotoBinaryFile = binaryFile;
					} else {
			...
			switch (this.sharedDataService.signOffFor) {
				case EnumService.SignOffType.INDUCTION:
					if (this.sharedDataService.offlineMode) {
						this.sharedDataService.checkInPostData.userSignaturePhotoFileName = fileName;
						this.sharedDataService.checkInPostData.userSignaturePhotoBinaryFile = binaryFile;
					} else {
				...
				case EnumService.SignOffType.WORK_PERMIT_DM:
					if (this.sharedDataService.offlineMode) {
						this.sharedDataService.signOffDetailsPostData.userSignaturePhotoFileName = fileName;
						this.sharedDataService.signOffDetailsPostData.userSignaturePhotoBinaryFile = binaryFile;
					} else {
