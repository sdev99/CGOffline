...
export class DashboardHeaderComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.observablesService
      .getObservable(
        EnumService.ObserverKeys.UPDATE_CURRENT_CHECKIN_LIST_IN_COMPONENT
      )
      .subscribe(() => {
        this.ngZone.run(() => {
    ...
    this.checkedPlaces.map((place, key) => {
      if (
        ...
        sharedDataService.currentSelectedCheckinPlace?.userCheckInDetailID
      ) {
  ...
  ngOnInit() {
    ...
    this.observablesService
      .getObservable(EnumService.ObserverKeys.REFRESH_CURRENT_CHECKIN_LIST)
      .subscribe((data) => {
        this.getUserCurrentCheckinDetails();
      });

    this.observablesService
      .getObservable(EnumService.ObserverKeys.NEW_CHECKED_IN)
      .subscribe((data) => {
        if (data) {
          this.getUserCurrentCheckinDetails();
        }
  ...
  setupCurrentCheckedInPlaces = (checkedInPlaces) => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify(checkedInPlaces)
    );
    ...
    if (checkedInPlaces && checkedInPlaces.length > 0) {
      if (this.selectedPlace) {
        ...
        checkedInPlaces.map((item: CheckedInDetailItem) => {
          if (
            this.selectedPlace.userCheckInDetailID === item.userCheckInDetailID
          ) {
            found = true;
            return;
          }
        ...
        if (!found) {
          const place =
            checkedInPlaces && checkedInPlaces.length > 0
              ? checkedInPlaces[0]
              : null;
          this.placedChange(place);
        }
  ...
  getUserCurrentCheckinDetails = () => {
    ...
    this.apiService
      .getUserCurrentCheckingDetails(this.user?.userId)
  ...
  ionViewDidEnter() {
    this.sharedDataService.checkAutoCheckOutForCurrentCheckin();
    this.setupCurrentCheckedInPlaces(this.sharedDataService.checkedInPlaces);
  }
  ...
  placedChange(place) {
    ...
    this.observablesService.publishSomeData(
      EnumService.ObserverKeys.CURRENT_CHECKED_IN_CHANGED
    );
  }
      ...
      .subscribe((res) => {
        this.navCtrl.navigateForward(["/checkinout-confirm"], {
          queryParams: {
            ...
            title:
              res["PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.YOU_ARE_CHECKING_OUT"],
  ...
  shouldShowNavigationPlaceBtns(btnType = 0) {
    ...
    if (
      btnType &&
      btnType === 2 &&
      this.currentCheckinPlaceIndex === this.checkedPlaces.length - 1
    ) {
      shouldShowBtn = false;
    }
    return (
      shouldShowBtn &&
      this.showPlaceNavigationBtns &&
      this.checkedPlaces &&
      this.checkedPlaces.length > 1
    );
  }
