...
export class OfflineApiService {
	...
	getDeviceOfflineFile(files) {
		return new Promise((resolve, reject) => {
			...
			if (isDemoFile) {
				resolve(['./assets/202108101317573870376.json']);
			} else {
	...
	getDeviceOfflineFileN(files) {
		return new Promise((resolve, reject) => {
			...
			if (isDemoFile) {
				this.filehandlerService.readJsonFile('./assets/202108101317573870376.json').subscribe((response) => {
					resolve(response);
				});
			} else {
				const accessID = this.sharedDataService.deviceUID;
				...
				const sampleZipFile = 'https://www.learningcontainer.com/wp-content/uploads/2020/05/sample-zip-file.zip';
				fetch(sampleZipFile)
					.then((res) => res.blob())
					.then((res) => {
						debugger;
				...
				files.map((obj: any) => {
					...
					fileTransfer.download(fileUrl, localFilePath, true, { headers: headers, ...headers }).then(
						(entry) => {
