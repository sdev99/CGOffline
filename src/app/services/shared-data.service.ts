...
export class SharedDataService {
  ...
  public checkAutoCheckOutForCurrentCheckin() {
    ...
    if (currentCheckinList && currentCheckinList.length > 0) {
      currentCheckinList.map((item) => {
        ...
        if (isAutoCheckOut) {
          const userProfileCurrentDate = moment(item.currentUTCDate).add(
            userProfileTimeZone?.timeDifference || 0,
            "minutes"
          );
          if (userProfileCurrentDate < autoCheckoutDateTime) {
            ...
            if (seconds > 0) {
              this.autocheckoutTimeoutRef[item.userCheckInDetailID] =
                setTimeout(() => {
                  if (this.checkedInPlaces) {
                    this.checkedInPlaces.some((place, placeIndex) => {
                      if (
                        place.userCheckInDetailID === item.userCheckInDetailID
                      ) {
      ...
      if (ifAlreadyCheckedinPlace) {
        ...
        this.translateService
          ...
          .subscribe((res) => {
            this.navCtrl.navigateForward(["/checkinout-confirm"], {
              queryParams: {
                headerTitle:
                  res["PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.CHECK_OUT"],
                title:
                  res[
                    "PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.YOU_ARE_CHECKING_OUT"
                  ],
                ...
                buttonTitle:
                  res["PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.CHECK_OUT_NOW"],
        ...
        apiService
          ...
          .subscribe(
            (response: Response) => {
              ...
              if (
                ...
                this.checkInPostData = {
                  ...
                  inventoryItemID:
                    this.checkInDetail.checkInEntityDetail.inventoryItemID,
                ...
                this.translateService
                  ...
                  .subscribe((res) => {
                    this.navCtrl.navigateForward(["/checkinout-confirm"], {
                      queryParams: {
                        headerTitle:
                          res["PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.CHECK_IN"],
                        title:
                          res[
                            "PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.YOU_ARE_CHECKING_IN"
                          ],
                        ...
                        locationCheckType:
                          EnumService.ConfirmForCheckType.CheckIn,
                      },
                      replaceUrl: true,
                    });
                  });
            ...
            (error: any) => {
              const errorField =
                error?.error?.ResponseException?.ValidationErrors[0].Field;
  ...
  uploadVideo = (selectedVideo, mimeType, callBack, progress) => {
    ...
    const url =
      this.apiBaseUrl + "/" + EnumService.ApiMethods.FormPhotoOrVideoUpload;
  ...
  public async saveFormAnswers(
    ...
    if (!environment.isWebApp) {
      ...
      if (
        ...
        this.translateService
          .get("SHARED_TEXT.ERRORS.ATLEAST_ONE_ANSWER_TO_BE_FILLED_IN")
          .subscribe((res) => {
        ...
        if (
          ...
          this.translateService
            .get("SHARED_TEXT.ERRORS.INCORRECT_DATA_FOUND_PLS_CHECK_ANSWERS")
            .subscribe((res) => {
              const errorMessage = res;
              callBack(false, errorMessage);
            });
            ...
            questions.some((question, questionIndex) => {
              ...
              const havQuestionAnswerObject: HavAnswerObject = {
                ...
                [EnumService.QuestionLogic.ActionTypeForForm.Notify]:
                  question[EnumService.QuestionLogic.ActionTypeForForm.Notify],
              ...
              switch (questionDisplayOrder) {
                case EnumService.HavFormFieldOrder.DateOfUsage:
                  if (question.value) {
                    havQuestionAnswerObject.dateOfUsage = moment(
                      question.value
                ...
                case EnumService.HavFormFieldOrder.PlannedTimeOfUsage:
                  if (question.value) {
                    isValueFilled = true;
                    havQuestionAnswerObject.plannedTimeOfUse = Number(
                      question.value
                    );
                  }
                  break;
                ...
                if (section.isAccidentReportSection) {
                  ...
                  switch (questionDisplayOrder) {
                    case EnumService.AccidentFormFieldOrder.AccidentDateTime:
                      if (control.value) {
                        isValueFilled = true;
                        answerObject.accidentDateTime = moment(
                          control.value
                    ...
                    case EnumService.AccidentFormFieldOrder.AccidentLocation:
                      const placeNotintheList =
                        formGroup.controls.placeNotintheList;
                    ...
                    case EnumService.AccidentFormFieldOrder.BodyPartEffected:
                      ...
                      StaticDataService.bodyParts.map((partGroup) => {
                        partGroup.parts.map((part) => {
                          const choiceControl =
                            bodyPartFormGroups[
                              UtilService.SubFCName(controlName, part.id)
                            ];
                          if (choiceControl) {
                  ...
                  switch (question.selectedAnswerTypeId) {
                    ...
                    case EnumService.CustomAnswerType.Matrix3DField:
                      if (control.value) {
                        ...
                        if (
                          ...
                          Object.keys(matrixSelectedValues).length > 0
                        ) {
                          isValueFilled = true;
                          ...
                          matrixRows.map((rowItem, rowIndex) => {
                            ...
                            if (
                              matrixColsAnswer &&
                              matrixColsAnswer.length > 0
                            ) {
                              matrixRowsAnswer.push({
                                matrixRowId: rowItem.matrixRowId,
                                matrixRowName: UtilService.findObj(
                    ...
                    case EnumService.CustomAnswerType.DateField:
                      if (control.value) {
                        isValueFilled = true;
                        answerObject.dateValue = moment(control.value).format(
                    ...
                    case EnumService.CustomAnswerType.TimeField:
                      if (control.value) {
                        isValueFilled = true;
                        answerObject.timeValue = moment(control.value).format(
                    ...
                    case EnumService.CustomAnswerType.DateTimeField:
                      if (control.value) {
                        isValueFilled = true;
                        answerObject.dateTimeValue = moment(
                          control.value
                        ).format(StaticDataService.dateTimeFormat);
                      }
                      break;
                  ...
                  if (question.shouldShowOptionalComment) {
                    const additionalControl =
                      formGroup.controls[
                        UtilService.FCNameAdditioanlNoteUq(controlName)
                      ];
  ...
  submitInductionCheckInData = async (apiService: ApiService) => {
    ...
    if (this.offlineMode) {
      ...
      if (this.dedicatedModeLocationUse.locationID) {
        ...
        entityData = await this.offlineManagerService.getDeviceProjectDetail(
        ...
        this.translateService
          ...
          .subscribe((res) => {
            this.navCtrl.navigateForward([failScreen], {
              queryParams: {
                title:
                  res["PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.YOU_CANNOT_CHECKIN"],
                errorTitle:
                  res["PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.NO_QUALIFICATION"],
                errorMessage:
                  res[
                    "PAGESPECIFIC_TEXT.CHECK_IN_AND_OUT.YOU_DO_NOT_HAVE_THE_REQUIRED_QUALIFICATIONS"
                  ],
        ...
        .subscribe((res) => {
          this.navCtrl.navigateForward([failScreen], {
            queryParams: {
              ...
              errorTitle: "",
              errorMessage:
                exception?.ResponseException?.ExceptionMessage ||
                exception?.Message,
              nextPage: nextScreen,
            },
            replaceUrl: true,
  ...
  submitPersonalModeSignoffData = async (apiService: ApiService) => {
    ...
    const onSuccessCallBack = (status, errorMessage = "") => {
      if (status) {
        if (
          this.signOffFor === EnumService.SignOffType.FORM_ACTIVITY ||
          this.signOffFor === EnumService.SignOffType.DOCUMENT_ACTIVITY
        ) {
          this.observablesService.publishSomeData(
            ...
            .subscribe((res) => {
              this.navCtrl.navigateForward([checkInFailPage], {
                queryParams: {
                  ...
                  nextPage,
                  pageTitle:
                    res["PAGESPECIFIC_TEXT.PAGESPECIFIC_TEXT.FORM_LIST"],
                },
                replaceUrl: true,
              });
            });
      ...
      if (
        ...
        if (
          ...
          if (this.dedicatedMode) {
            ...
            if (this.offlineMode) {
              this.offlineManagerService
                .getSignOffFormDetail(inductionContentItem.formID)
                .then((response) => {
                  onSuccess(response);
                });
            } else {
