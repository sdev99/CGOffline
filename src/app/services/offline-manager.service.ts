...
export class OfflineManagerService {
        ...
        StaticDataService.offlineTables.map((data) => {
            if (data.table_name) {
                ...
                this.dbQuery(query, []).then(
                    ...
                    (err) => {
                        console.error(
                            data.table_name + " Unable to execute sql: ",
                            err
                        );
                    }
                );
            ...
            if (data.table_name) {
                this.emptyTable(data.table_name)
                    .then(() => {
                        ...
                        if (
                            emptiedCount ===
                            StaticDataService.offlineTables.length
                        ) {
                            callBack && callBack();
                        }
                    })
    ...
    insertOfflineData = async (
        ...
        const deviceUserQualificationList =
            offlineData.deviceUserQualificationList;
        if (deviceUserQualificationList) {
            for (
                let index = 0;
                index < deviceUserQualificationList.length;
                index++
            ) {
        ...
        const deviceAvailableDocumentList =
            offlineData.deviceAvailableDocumentList;
        if (deviceAvailableDocumentList) {
            for (
                let index = 0;
                index < deviceAvailableDocumentList.length;
                index++
            ) {
        ...
        const deviceArchivedDocumentList =
            offlineData.deviceArchivedDocumentList;
        if (deviceArchivedDocumentList) {
            for (
                let index = 0;
                index < deviceArchivedDocumentList.length;
                index++
            ) {
        ...
        if (deviceAvailableFormList) {
            for (
                let index = 0;
                index < deviceAvailableFormList.length;
                index++
            ) {
                const value: DeviceAvailableFormDetail =
                    deviceAvailableFormList[index];
        ...
        if (deviceArchivedFormList) {
            for (
                let index = 0;
                index < deviceArchivedFormList.length;
                index++
            ) {
                const value: DeviceArchivedFormDetail =
                    deviceArchivedFormList[index];
                const condition: any = {
                    documentID: value.documentID,
                };
                if (value.locationID > 0) {
        ...
        if (deviceLiveWorkPermitList) {
            for (
                let index = 0;
                index < deviceLiveWorkPermitList.length;
                index++
            ) {
        ...
        if (deviceFormAttachmentList) {
            for (
                let index = 0;
                index < deviceFormAttachmentList.length;
                index++
            ) {
        ...
        if (deviceEvacuationList) {
            for (let index = 0; index < deviceEvacuationList.length; index++) {
                const value: DeviceEvacuationDetail =
                    deviceEvacuationList[index];
        ...
        if (deviceInventoryItemList) {
            for (
                let index = 0;
                index < deviceInventoryItemList.length;
                index++
            ) {
                const value: DeviceInventoryItemDetail =
                    deviceInventoryItemList[index];
                const condition: any = {
                    companyID: value.companyID,
        ...
        const deviceUserCheckinDetailList =
            offlineData.deviceUserCheckinDetailList;
        if (deviceUserCheckinDetailList) {
            for (
                let index = 0;
                index < deviceUserCheckinDetailList.length;
                index++
            ) {
        ...
        const deviceCheckInInductionList =
            offlineData.deviceCheckInInductionList;
        if (deviceCheckInInductionList) {
            for (
                let index = 0;
                index < deviceCheckInInductionList.length;
                index++
            ) {
        ...
        if (deviceCheckInGuestInductionItemList) {
            for (
                ...
                const condition: any = {
                    checkInInductionGuestID: value.checkInInductionGuestID,
                    checkInInductionGuestItemID:
                        value.checkInInductionGuestItemID,
                    documentID: value.documentID,
                    formID: value.formID,
                };
        ...
        if (deviceCompanyUserList) {
            for (let index = 0; index < deviceCompanyUserList.length; index++) {
                const value: DeviceCompanyUserDetail =
                    deviceCompanyUserList[index];
                const condition: any = {
                    userId: value.userId,
                };
        ...
        const deviceCompanyUserGroupList =
            offlineData.deviceCompanyUserGroupList;
        if (deviceCompanyUserGroupList) {
            for (
                let index = 0;
                index < deviceCompanyUserGroupList.length;
                index++
            ) {
        ...
        if (deviceHAVManufacturerList) {
            for (
                let index = 0;
                index < deviceHAVManufacturerList.length;
                index++
            ) {
        ...
        if (deviceHazardItemList) {
            for (let index = 0; index < deviceHazardItemList.length; index++) {
                const value: DeviceHazardItemDetail =
                    deviceHazardItemList[index];
                const condition: any = {
                    companyID: value.companyID,
                    companyRiskItemID: value.companyRiskItemID,
        ...
        if (deviceLocationItemList) {
            for (
                let index = 0;
                index < deviceLocationItemList.length;
                index++
            ) {
        ...
        if (deviceAccidentTypeList) {
            for (
                let index = 0;
                index < deviceAccidentTypeList.length;
                index++
            ) {
        ...
        if (deviceRiskAssessmentProbabilityOptions) {
            for (
                ...
                const value: any =
                    deviceRiskAssessmentProbabilityOptions[index];
                const condition: any = {
        ...
        if (deviceRiskAssessmentSeverityOptions) {
            for (
                ...
                const condition: any = {
                    riskAssessmentSeverityOptionId:
                        value.riskAssessmentSeverityOptionId,
                };
                await this.insertData(
                    "DeviceRiskAssessmentSeverityOptions",
                    value,
    ...
    getAvailableDocuments(folderId, entity: DeviceEntityDetail) {
        return new Promise((resolve, reject) => {
            ...
            if (folderId) {
                ...
                condition =
                    condition + " AND " + this.emptyCond("parentFolderID");
            }
    ...
    getAvailableForms(folderId, entity: DeviceEntityDetail) {
        return new Promise((resolve, reject) => {
            ...
            if (folderId) {
                ...
                condition =
                    condition + " AND  " + this.emptyCond("parentFormFolderID");
            }
                ...
                this.dbQuery(qxpiredQuery, [])
                    .then((res: any) => {
                        if (res.rows?.length > 0) {
                            ...
                            const finalList =
                                filteredList.concat(archivedPermitList);
    ...
    getDeviceRiskAssessmentProbabilityOptions() {
        return new Promise((resolve, reject) => {
            const query =
                "SELECT * FROM DeviceRiskAssessmentProbabilityOptions";
            this.dbQuery(query, [])
                .then((res: any) => {
                    if (res.rows?.length > 0) {
                        resolve(this.convertToArray(res.rows));
    ...
    getAnswerChoiceEntityByQRCode(qrCode) {
        return new Promise(async (resolve, reject) => {
            ...
            if (inventryItemsRes && inventryItemsRes.rows?.length > 0) {
                const inventoryItem = this.convertToObject(
                    inventryItemsRes.rows
                );
    ...
    getDeviceUserLastCheckinDetail(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (isGuest) {
                ...
                condition =
                    condition + ' AND userId = "' + userIdOrGuestPhone + '"';
                query =
    ...
    getDeviceUserCheckinDetails(
        ...
        return new Promise((resolve, reject) => {
            ...
            this.dbQuery(query, [])
                .then(async (res: any) => {
                    const deviceUserCheckinDetails = this.convertToArray(
                        res.rows
                    );
                    ...
                    const deviceAssginedEntity = localStorage.getItem(
                        EnumService.LocalStorageKeys
                            .DEDICATED_MODE_ASSIGNED_ENTITIES
                    );
                    if (deviceAssginedEntity) {
                        try {
                            ...
                            list.map((obj) => {
                                if (obj.locationID) {
                                    deviceAssginedLocationIds.push(
                                        obj.locationID
                                    );
                                } else if (obj.projectID) {
                                    deviceAssginedProjectIds.push(
                                        obj.projectID
                                    );
                                } else if (obj.inventoryItemID) {
                                    deviceAssginedInventoryItemIds.push(
                                        obj.inventoryItemID
                                    );
                                }
                            });
                    ...
                    if (res.rows?.length > 0) {
                        if (
                            ...
                            deviceUserCheckinDetails.some((item) => {
                                const locationIsAssigned =
                                    (item.locationID &&
                                        deviceAssginedLocationIds.indexOf(
                                            item.locationID
                                        ) !== -1) ||
                                    (item.projectID &&
                                        deviceAssginedProjectIds.indexOf(
                                            item.projectID
                                        ) !== -1) ||
                                    (item.inventoryItemID &&
                        ...
                        if (
                            isAlreadyCheckinToThisEntity ||
                            !isSimultaneousCheckInAllowed
                        ) {
                            // let entityData: any;
                            let Field = "";
                            let Message = "";
            ...
            const onSuccess = async (deviceUserCheckinDetails) => {
                ...
                if (entity.locationID) {
                    entityData = await this.getDeviceLocationDetail(
                        entity.locationID
                    );
                } else if (entity.projectID) {
                    entityData = await this.getDeviceProjectDetail(
                        entity.projectID
                    );
                } else if (entity.inventoryItemID) {
                ...
                if (inductionDetail) {
                    ...
                    if (lastCheckinForCurrentLocation) {
                        if (lastCheckinForCurrentLocation.showInductionSteps) {
                            ...
                            const renewEveryPeriodType =
                                inductionDetail.renewEveryPeriodType;
                            const renewEveryPeriod =
                                inductionDetail.renewEveryPeriod;
    ...
    getDeviceCheckInGuestInductionItems(checkInInductionGuestID: string) {
        return new Promise((resolve, reject) => {
            let condition =
                "checkInInductionGuestID=" + checkInInductionGuestID;
    ...
    getModelList(companyId, typeId) {
        return new Promise((resolve, reject) => {
            const condition =
                "companyID=" + companyId + " AND havTypeID=" + typeId;
    ...
    getFormDetail(formId, formVersionId, formVersionNo, folderId = null) {
        return new Promise((resolve, reject) => {
            ...
            const query =
                "SELECT * FROM DeviceFormDetails" + " WHERE " + condition;

            this.dbQuery(query, [])
                .then((res: any) => {
                    if (res.rows?.length > 0) {
                        const formDetail = this.convertToObject(res.rows);
                        this.getDeviceFormAttachments(formId).then(
                            (attachments: any) => {
                                let formAttachments = [];
                                if (attachments) {
    ...
    getFormBuilderDetail(formId, formVersionId, formVersionNo) {
        return new Promise((resolve, reject) => {
            let condition =
                ...
                formVersionNo;
            const query =
                "SELECT * FROM DeviceFormBuilderDetails" +
                " WHERE " +
                condition;
                ...
                .then((res: any) => {
                    if (res.rows?.length > 0) {
                        ...
                        if (obj.total_count > 0) {
                            data.documentTitle =
                                data.documentTitle + " " + obj.total_count;
                        }
                    }
    ...
    insertCheckinDetails = async (data) => {
        ...
        if (checkinQualifications && checkinQualifications.length > 0) {
            const userQualifications: any =
                await this.getDeviceUserQualifications(data.userId);
            if (userQualifications && userQualifications.length > 0) {
                ...
                checkinQualifications.map((checkinQualObj) => {
                    userQualifications.some((userQualObj) => {
                        if (
                            checkinQualObj.qualificationID ===
                            userQualObj.qualificationID
                        ) {
                            if (
                ...
                if (
                    matchedQualificationCount === checkinQualifications.length
                ) {
                    userQualified = true;
                }
            }
                ...
                this.dbQuery(query, dataVals)
                    .then(async (res: any) => {
                        ...
                        if (data.firstAndLastName) {
                            evacCondition["firstAndLastName"] =
        ...
        if (!data.isGuestReturning) {
            ...
            const guestUser = {
                ...
                guestPhotoImageVideoFileId:
                    data.guestPhotoImageVideoFileId || "",
            ...
            this.dbQuery(query, dataVals)
                .then(async (res: any) => {
                    ...
                    const evacuationData = {
                        ...
                        userPhotoImageVideoFileId:
                            data.guestPhotoImageVideoFileId || 0,
                    };

                    let evacCondition = {};
                    if (evacuationData.inventoryItemID) {
                        evacCondition["inventoryItemID"] =
                            evacuationData.inventoryItemID;
                    ...
                    if (evacuationData.firstAndLastName) {
                        evacCondition["firstAndLastName"] =
                            evacuationData.firstAndLastName;
                    }
                    this.insertData(
                        "DeviceEvacuations",
                        evacuationData,
    ...
    insertCheckOutDetails = (data, cond) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            let condition =
                "deviceUserCheckinDetailId = " + userCheckInDetailID;
            let query =
            ...
            this.dbQuery(query, dataVals)
                .then(async (res: any) => {
                    ...
                    const checkinDetails: any = await this.dbQuery(
                        checinDetailQuery
                    );

                    if (checkinDetails && checkinDetails.rows.length > 0) {
                        const checkinDetail = this.convertToObject(
                            checkinDetails.rows
                        );
                        ...
                        removeCondition =
                            ...
                            '"';
                        let removeQuery =
                            "DELETE from DeviceEvacuations WHERE " +
                            removeCondition;
                        this.dbQuery(removeQuery).finally(() => {
                            addLastCheckinDetails(checkinDetail);
                        });
                    } else {
                        resolve(res);
                    }
            ...
            const addLastCheckinDetails = async (checkinDetail) => {
                const lastCheckinData = {
                    ...
                    todayDate: "",
                    userCheckInDetailID: userCheckInDetailID,
                    userId: checkinDetail.userId,
                    isOfflineDone: true,
                };
                ...
                if (getLastCheckinDetail) {
                    ...
                    if (checkinDetail.inventoryItemID) {
                        lastCheckinData["inventoryItemID"] =
                            checkinDetail.inventoryItemID;
                    } else if (checkinDetail.locationID) {
                        lastCheckinData["locationID"] =
                            checkinDetail.locationID;
    ...
    insertCheckOutDetails_Guest = (data, cond: any = {}) => {
        const deviceGuestUserCheckinDetailId =
            cond.deviceGuestUserCheckinDetailId;
        ...
        return new Promise((resolve, reject) => {
            ...
            let condition =
                "deviceGuestUserCheckinDetailId = " +
                deviceGuestUserCheckinDetailId;
            let query =
            ...
            this.dbQuery(query, dataVals)
                .then(async (res: any) => {
                    //Get checkin details
                    const checinDetailQuery =
                        "SELECT * FROM DeviceGuestUserCheckinDetails WHERE " +
                        condition;
                    // If user checkout then remove it from Evacuation list
                    const checkinDetails: any = await this.dbQuery(
                        checinDetailQuery
            ...
            const addLastCheckinDetails = async (checkinDetail) => {
                const lastCheckinData = {
                    ...
                    showInductionSteps: false,
                    todayDate: "",
                    userCheckInDetailID: deviceGuestUserCheckinDetailId,
                    isOfflineDone: true,
                };
                ...
                .then((res: any) => {
                    ...
                    if (signOffRes && signOffRes.total_result > 0) {
                        ...
                        this.dbQuery(checkinQuery)
                            .then((res1: any) => {
                                const checkinRes = this.convertToObject(
                                    res1.rows
                                );
                                if (checkinRes && checkinRes.total_result > 0) {
                                    ...
                                    this.dbQuery(guestCheckinQuery)
                                        .then((res2: any) => {
                                            const guestCheckinRes =
                                                this.convertToObject(res2.rows);
                                            if (
                                                guestCheckinRes &&
                                                guestCheckinRes.total_result > 0
                                            ) {
                                                resolve(true);
                                            } else {
                                                resolve(false);
                ...
                submittedFormData.formattedSections?.map((section) => {
                    section.answerData?.map((questionAsnwerObj) => {
                        if (section.isAccidentReportSection) {
                            if (
                                questionAsnwerObj.answerData
                                    ?.accidentAttachmentFileId
                            ) {
                                questionAsnwerObj.answerData[
                                    "accidentAttachmentFileName"
                                ] =
                                    uploadedFiles[
                                        "imageVideoFileId_" +
                                            questionAsnwerObj.answerData
                                ...
                                delete questionAsnwerObj.answerData
                                    .accidentAttachmentFileId;
                            }
                            if (
                                questionAsnwerObj.arAnswerData
                                    ?.accidentAttachmentFileId
                            ) {
                                questionAsnwerObj.arAnswerData[
                                    "accidentAttachmentFileName"
                                ...
                                delete questionAsnwerObj.arAnswerData
                                    .accidentAttachmentFileId;
                            }
                        } else {
                            if (
                ...
                submittedFormData.questionAnswers?.map((questionAsnwerObj) => {
                    if (questionAsnwerObj.imageVideoFileId) {
                        questionAsnwerObj["imageVideoFileName"] =
                            uploadedFiles[
                                "imageVideoFileId_" +
                                    questionAsnwerObj.imageVideoFileId
                            ];
    ...
    async prepareJsonFileForPost(deviceId, offlineApiService) {
        ...
        return new Promise(async (resolve, reject) => {
                        ...
                        if (response.Result) {
                            ...
                            if (uploadedFiles) {
                                uploadedFiles.map((uploadedFileData) => {
                                    ...
                                    const newFileName =
                                        uploadedFileData.newFileName;

                                    const originalFileNameParts =
                                        originalFileName.split("|");
                                    if (
                                        originalFileNameParts &&
                                        originalFileNameParts.length > 0
                                    ) {
                                        const imageVideoFileId =
                                            originalFileNameParts[0];
                                        uploadedFilesObj[
                                            "imageVideoFileId_" +
                                                imageVideoFileId
                                        ] = newFileName;
            ...
            const getSignOffDetails = async (uploadedFiles = {}) => {
                const signOffDetails: any =
                    await this.getOfflineSignoffDetails();
                ...
                if (signOffDetails && signOffDetails.length > 0) {
                    const addToSignOffDetailsList = (dataObj) => {
                        ...
                        if (
                            signOffDetailsList.length === signOffDetails.length
                        ) {
                            postJsonFileData.personalModeSignOffDetails =
                                signOffDetailsList;
            ...
            const getUserCheckinDetails = async (uploadedFiles) => {
                const checkinDetails: any =
                    await this.getOfflineUserCheckinDetails();
                ...
                if (checkinDetails && checkinDetails.length > 0) {
                    ...
                    const addToCheckinDetailList = (obj) => {
                        checkinDetailsList.push({
                            ...
                            digitalInkSignatureFileName:
                                uploadedFiles[
                                    "imageVideoFileId_" +
                                        obj.digitalInkSignatureImageVideoFileId
                                ] || "",
                            digitalInkSignatureBinaryFile: "",
                            digitalInkSignature:
                                uploadedFiles[
                            ...
                            userSignaturePhotoFileName:
                                uploadedFiles[
                                    "imageVideoFileId_" +
                                        obj.userSignaturePhotoImageVideoFileId
                                ] || "",
                            userSignaturePhotoBinaryFile: "",
                            userSignaturePhoto:
                                uploadedFiles[
                            ...
                            offlineUserCheckInDetailID:
                                obj.deviceUserCheckinDetailId,
                            processInductionsteps: obj.processInductionsteps,
                        });
                        if (
                            checkinDetails.length === checkinDetailsList.length
                        ) {
                            postJsonFileData.checkInDetails =
                                checkinDetailsList;
                            isDataAvailableForPost = true;
                    ...
                    checkinDetails.map((checkinDetailObj) => {
                        if (
                            ...
                            checkinDetailObj.formSubmitDataId.forEach(
                                (formSubmitDataId) => {
                                    this.getOfflineSubmittedFormData(
                                        formSubmitDataId
                                    ).then((submittedFormData: any) => {
                                        ...
                                        checkinDetailObj.formAnswerData.push(
                                            submittedFormData
                                        );
                                        ...
                                        if (
                                            checkinDetailObj.formSubmitDataId
                                                .length === countSubmittedForm
                                        ) {
                                            addToCheckinDetailList(
                                                checkinDetailObj
                                            );
                                        }
                                    });
            ...
            const getGuestCheckinDetails = async (uploadedFiles) => {
                const checkinDetails: any =
                    await this.getOfflineGuestCheckinDetails();
                    ...
                    const addToCheckinDetailList = (obj) => {
                        checkinDetailsList.push({
                            ...
                            guestPhotoFileName:
                                uploadedFiles[
                                    "imageVideoFileId_" +
                                        obj.guestPhotoImageVideoFileId
                                ] || "",
                            ...
                            formAnswerData: obj.formAnswerData || "",
                            offlineUserCheckInDetailID:
                                obj.deviceGuestUserCheckinDetailId,
                            processInductionsteps: obj.processInductionsteps,
                        });
                        if (
                            checkinDetails.length === checkinDetailsList.length
                        ) {
                            postJsonFileData.checkInDetails_Guest =
                                checkinDetailsList;
                            isDataAvailableForPost = true;
            ...
            const getUserCheckoutDetails = async () => {
                const checkoutDetails: any =
                    await this.getOfflineUserCheckoutDetails();
            ...
            const getGuestCheckoutDetails = async () => {
                ...
                if (checkoutDetails && checkoutDetails.length > 0) {
                    ...
                    postJsonFileData.checkOutDetails_Guest =
                        checkoutDetailsList;
                    isDataAvailableForPost = true;
